// File: components/Home/LatestPost.tsx
import React, {
  useContext,
  useEffect,
  useState,
...
// Forward ref interface for parent control
...
// Memoized component following performance guidelines with forwardRef
const LatestPost = React.memo(
  forwardRef<LatestPostRef, LatestPostProps>(function LatestPost(
    { search },
    ref
  ) {
    ...
    const colors = useMemo(
      () => ({
        ...
        tabBackground: isDarkMode ? '#2a2a2a' : '#f8f9fa',
        tabBackgroundActive: Colors.PRIMARY,
        onPrimary: '#ffffff',
        searchContainerBackground: isDarkMode ? '#1a1a1a' : '#ffffff',
        tabsContainerBackground: isDarkMode ? '#1a1a1a' : '#ffffff',
      }),
      [isDarkMode]
    )
      ...
      () =>
        StyleSheet.create({
          ...
          tabsContainer: {
            ...
            marginBottom: 12,
            backgroundColor: colors.tabsContainerBackground,
            paddingVertical: 8,
          },
          tabButton: {
            paddingVertical: 10,
    ...
    const latestPostsQuery = useAllPosts(
      user?.email,
      false,
    ...
    // ДОБАВЕНО: Expose refresh method to parent component
    useImperativeHandle(
      ref,
      () => ({
        refresh: () => {
